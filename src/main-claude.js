...
class PPLWidget {
  constructor(containerElement, config = {}) {
    ...
    this.init();
    // Inicializace mobilního layoutu po renderování
    if (this.isMobile) {
  ...
  renderWidgetHTML() {
    this.container.innerHTML = `
     <div class="ppl-widget ${this.isMobile ? 'ppl-mobile' : 'ppl-desktop'}">
  ...
  async init() {
    ...
    this.initializeCountryFilter(); // PŘIDÁNO
    setTimeout(() => {
  ...
  // Nové metody pro responsive chování
  updateDeviceClass() {
    if (this.isMobile) {
      this.container.classList.add('ppl-mobile');
      this.container.classList.remove('ppl-desktop');
    } else {
      this.container.classList.add('ppl-desktop');
      this.container.classList.remove('ppl-mobile');
    }
  }
  ...
  updateLayoutForDevice() {
    if (this.isMobile) {
      this.initMobileLayout();
      // Skryj desktop-only elementy
      const desktopElements = this.container.querySelectorAll('.desktop-only');
      desktopElements.forEach((el) => (el.style.display = 'none'));

      // Zobraz mobile-only elementy
      const mobileElements = this.container.querySelectorAll('.mobile-only');
      mobileElements.forEach((el) => (el.style.display = ''));
    } else {
      this.initDesktopLayout();
      // Zobraz desktop-only elementy
      const desktopElements = this.container.querySelectorAll('.desktop-only');
      desktopElements.forEach((el) => (el.style.display = ''));

      // Skryj mobile-only elementy
      const mobileElements = this.container.querySelectorAll('.mobile-only');
  ...
  initMobileLayout() {
    // Přidej mobilní toggle button pro přepínání mapa/seznam
    if (!this.container.querySelector('.ppl-mobile-toggle')) {
      const mobileToggle = document.createElement('div');
      mobileToggle.className = 'ppl-mobile-toggle mobile-only';
      mobileToggle.innerHTML = `
      <button class="ppl-mobile-toggle-btn active" data-view="list">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M3 13h2v-2H3v2zm0 4h2v-2H3v2zm0-8h2V7H3v2zm4 4h14v-2H7v2zm0 4h14v-2H7v2zM7 7v2h14V7H7z"/>
        </svg>
        Seznam
      </button>
      <button class="ppl-mobile-toggle-btn" data-view="map">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    ...
    `;
      ...
      // Event listenery pro přepínání
      mobileToggle.addEventListener('click', (e) => {
        const button = e.target.closest('.ppl-mobile-toggle-btn');
        if (button) {
  ...
  initDesktopLayout() {
    // Odstraň mobilní prvky
    const mobileToggle = document.querySelector('.ppl-mobile-toggle');
    if (mobileToggle) {
      mobileToggle.remove();
    }
  }
  ...
  switchMobileView(view) {
    const toggleButtons = document.querySelectorAll('.ppl-mobile-toggle-btn');
    const resultsSection = this.container.querySelector('.ppl-sidebar');
    const mapContainer = this.container.querySelector('.ppl-map-container');
